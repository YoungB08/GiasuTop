...
function getCounterpartName(activeClassroom: any, role?: string) {
  if (!activeClassroom) return "Phòng học";
  if (role === "STUDENT") return activeClassroom.tutor_name || "Gia sư";
  if (role === "TUTOR") return activeClassroom.student_name || "Học viên";
  return activeClassroom.tutor_name || activeClassroom.student_name || "Phòng học";
}
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 bg-neutral-900 px-3 sm:px-5">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-semibold">Lớp với {counterpartName}</div>
              <button
                type="button"
                onClick={handleCopyRoom}
                className="mt-0.5 text-left text-[11px] text-neutral-400 hover:text-white"
              >
                ID {classroomId} {copied ? "- Đã copy" : ""}
              </button>
            </div>
          </div>
        ...
        <footer className="shrink-0 border-t border-white/10 bg-neutral-900 px-2 py-2 sm:px-5">
          <div className="grid w-full grid-cols-4 gap-1 lg:flex lg:items-center lg:justify-center lg:gap-2">
            ...
            >
              <PhoneOff size={18} />
              <span className="max-w-full truncate px-1">Thoát</span>
            </button>
        ...
        <aside className="absolute inset-y-0 right-0 z-30 flex w-full max-w-sm flex-col border-l border-white/10 bg-neutral-900 shadow-2xl sm:relative sm:w-80">
          <div className="flex h-14 shrink-0 items-center justify-between border-b border-white/10 px-4">
            <div className="text-sm font-semibold">
              {sidePanel === "chat" ? "Trò chuyện" : sidePanel === "participants" ? `Thành viên (${participantCount})` : "Thiết bị"}
            </div>
            ...
            <>
              <div className="flex-1 space-y-3 overflow-y-auto p-4">
                {chatMessages.length === 0 ? (
                  <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4 text-center text-xs text-neutral-400">Chưa có tin nhắn</div>
                ) : (
                    ...
                    return (
                      <div key={message.id || `${message.senderSocketId}-${message.createdAt}`} className={cx("flex flex-col", mine ? "items-end" : "items-start")}>
                        <div className="mb-1 max-w-[85%] truncate text-[10px] text-neutral-500">
                          {mine ? "Bạn" : message.senderName || message.sender} - {message.time}
                        </div>
          ...
          {sidePanel === "settings" && (
            <div className="flex-1 space-y-5 overflow-y-auto p-4">
              ...
              <div className="rounded-lg border border-white/10 bg-white/[0.03] p-3 text-xs text-neutral-400">
                {connected ? "Đã kết nối phòng học" : "Đang kết nối phòng học"}
              </div>
            </div>
          )}
        </aside>
  ...
  return (
    <div className="group relative flex h-full min-h-0 items-center justify-center overflow-hidden rounded-lg border border-white/10 bg-neutral-900">
      ...
      <div className="absolute left-3 top-3 flex gap-2">
        {tile.participant.isHost && <Badge>Chủ phòng</Badge>}
        {tile.participant.isScreenSharing && <Badge tone="green">Chia sẻ</Badge>}
        {tile.participant.isHandRaised && <Badge tone="amber">Giơ tay</Badge>}
      </div>

      <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between gap-2">
        <div className="min-w-0 rounded-md bg-black/60 px-2 py-1 text-xs font-semibold text-white backdrop-blur">
          <span className="truncate">{tile.participant.userName}{tile.isLocal ? " (Bạn)" : ""}</span>
        </div>
    ...
    <div className="mb-2 flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.03] p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-[10px] uppercase tracking-wide text-neutral-500">
          ...
          {participant.isHost && <span>Chủ phòng</span>}
        </div>
      </div>
