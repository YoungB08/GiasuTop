...
const UpsertTutorProfileSchema = z.object({
  ...
  subjectsToTeach: z.array(z.string().min(1).max(60)).min(1).max(30),
...
export async function upsertMyTutorProfile(req: AuthedRequest, res: Response): Promise<any> {
  ...
  const isVerified = profileRows[0]?.is_verified;
  if (isVerified !== "APPROVED") {
    return res.status(403).json({
      success: false,
      message: "Ho so cua ban chua duoc phe duyet. Vui long gui CCCD va minh chung truoc.",
    });
  }

  const input = UpsertTutorProfileSchema.parse(req.body);
  const subjectsCsv = input.subjectsToTeach.map((s) => s.trim()).filter(Boolean).join(",");

  await pool.query(
    "INSERT INTO tutor_profiles (user_id, bio, school, major, year_of_study, hourly_rate, subjects_to_teach, card_gradient, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'APPROVED') " +
      "ON DUPLICATE KEY UPDATE bio=VALUES(bio), school=VALUES(school), major=VALUES(major), year_of_study=VALUES(year_of_study), hourly_rate=VALUES(hourly_rate), subjects_to_teach=VALUES(subjects_to_teach), card_gradient=VALUES(card_gradient)",
    [
      req.user!.id,
      input.bio ?? null,
      input.school ?? null,
      ...
      subjectsCsv,
      input.cardGradient ?? "bg-gradient-to-r from-blue-600 via-indigo-600 to-[#13519c]",
    ]
  );

  return res.json({ success: true, message: "Cap nhat ho so day hoc thanh cong." });
}
...
export async function uploadMyTutorDocuments(req: AuthedRequest, res: Response): Promise<any> {
  ...
  try {
    ...
    if (currentStatus === "APPROVED") {
      ...
      await connection.query(
        "INSERT INTO tutor_profiles (user_id, bio, school, major, year_of_study, hourly_rate, subjects_to_teach, is_verified, reject_reason) " +
          "VALUES (?, '', 'Chưa cập nhật', 'Chưa cập nhật', 'Sinh viên năm 1', 150000, '', 'PENDING', NULL) " +
          "ON DUPLICATE KEY UPDATE is_verified='PENDING', reject_reason=NULL",
        [req.user!.id]
      );
    }
